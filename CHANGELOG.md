...
기획 **품질** 3종 — 도구가 아니라 산출되는 기획 내용 자체를 올리는 릴리스.

- **품질 점검 advisory** (#105) — prd·feature 문서에서 검증 가능성을 결정적으로
  짚는다: 수용 기준·목표의 측정 불가 표현("빠르게·쉽게·직관적")과 무숫자, 빈약한
  섹션, PRD 비범위 부재, 만성 누락 영역(실패 처리·권한·빈 상태·동시성·기존 데이터)
  미언급. 게이트 아님 — "의도한 것이면 그대로 두세요".
- **꼬리 질문** (#106) — 인터뷰 답변이 얕은 곳만 골라 AI 가 최대 3개 보강 질문을
  만든다("충분하면 만들지 마라"). 답변은 기존 경로로 초안 생성에 합류.
  인터뷰 화면의 "보강 질문 받기" 버튼, 보강 배지.
- **카운슬 비평** (#106) — 엔지니어·디자이너·회의론자 3관점 비평을 AI 1회 호출로
  받아 **제안 카드**(질문)로 붙인다(최대 6개, 근거 라벨 `카운슬 · 엔지니어`).
  설정에서 on/off (기본 켬, 끄면 버튼 숨김 + API 409). 섹션 제목이 안 맞는 비평은
  붙이지 않고 버린다.
- fix: 질문 카드 수락이 섹션을 문서로 확정시키던 문제 — 답하기는 카드만 정리하고,
  확정은 그 섹션 자신의 카드로만.

## 1.7.1

- **브리프 정합성 advisory** (#101) — feature 문서가 연결된 브리프에 없는 이름
  (코드 스팬·CamelCase·snake_case·경로류)을 참조하면 "신규 도입인가, 오타인가"를
  조용히 알려준다. 결정적·AI 무관, 수락된 브리프 섹션 기준. **게이트가 아니다** —
  컴파일 lint(waive·핸드오프 게이트)와 별개 채널이고, 브리프가 없으면 꺼진다.
- **작은 기능 구현 발주** (#102) — feature 문서의 "구현 발주" 버튼: 수락된 기획 +
  브리프 맥락(수락분 전문)을 코딩 에이전트용 발주 마크다운으로 내보낸다. 수용 기준은
  체크박스로, advisory 의 미확인 이름은 에이전트 질문거리로 명시 이관. 게이트 없음 —
  작은 기능은 수락분이 곧 발주다. 브리프 없이도 동작.
- 브리프 수집기: 모노레포 엔트리 관례(`api/src/index.ts`·`server/`·`backend/`·
  `web/`·`frontend/`) 추가 — 워크스페이스형 레포에서 엔트리 head 가 0개로 잡히던 것 보완.

## 1.7.0

- **작은 기능 기획 + 프로젝트 브리프 (P0)** (#99) — 이미 있는 제품에 작은 기능을 기획하는
  별도 흐름. 새 문서 타입 2종:
  - `feature`(작은 기능 기획) — 체인 독립 단독 문서. 인터뷰 7문(배경·현재 동작·변경·
    규칙·수용 기준·영향 범위·예외). 선택적으로 브리프를 부모로 연결하면 기존 부모 컨텍스트
    상속으로 실제 스택·모듈 이름이 생성에 반영된다.
  - `brief`(프로젝트 브리프) — 스택·아키텍처·네이밍 컨벤션·기존 기능 목록·핵심 용어·제약.
    **레포 폴더에서 추출**: 로컬 경로를 주면 README·매니페스트·디렉터리 구조·엔트리 파일
    head 만 결정적으로 수집(코드 전량 X, 심볼릭 링크 미추적)해 AI 가 제안 섹션으로 압축.
    수락해야만 문서가 되는 건 다른 문서와 같다. 도커 등 폴더 접근 불가 시 안내 후
    직접 작성/붙여넣기 경로로 우회.
- 시작 화면 "이미 있는 제품에 기능 추가" 진입, ⌘N 시트 옵션, 기타 문서 내비 그룹.
- fix(nav): 체인 고정 노드가 없는 타입의 첫 문서가 사이드바에서 사라지던 문제(#88 계열).
- 미룬 것: 정합성 advisory lint(브리프에 없는 모듈 경고), feature 전용 핸드오프 팩 — P1.

## 1.6.10

- **링크 ref 쓰기 시점 검증** (#97) — `/api/items/:id/link`·`/link-feature`·`/step-page` 와
  MCP `drafting-add-item` 의 링크 ref 를 저장 전에 검증한다. 대상 후보가 아직 없으면
  통과(체인 정방향 작성 — 기능명세가 IA 이전에 PG-nn 을 참조하는 경우), 후보가 있는데
  안 맞으면 유효 ref 목록과 함께 거절. 컴파일 E-BROKEN-REF 는 최종 무결성 망으로 유지.
- 미정의 웹 경로가 빈 화면 대신 시작 화면으로 리다이렉트 (#97).
- **릴리스 CI: macOS 러너 `macos-15` 고정** — macos-latest 의 macOS 26 전환으로
  electron-builder 임시 키체인이 깨져 v1.6.9 dmg 빌드가 실패한 것을 복구.
  (v1.6.9 는 npm·GHCR 로만 나감 — 데스크톱 인스톨러는 이 버전부터 다시 정상.)

## 1.6.9

- **라이선스: MIT → AGPLv3 + 상표·귀속 조건** — 무료·오픈소스는 그대로. 달라지는 것은
  수정본을 네트워크 서비스로 제공할 때의 소스 공개 의무(AGPL 13조)와 상표 사용 금지(7e)·
  귀속 표기(7b). 이 릴리스부터 npm 패키지·인스톨러·GHCR 이미지에 새 라이선스가 담긴다.
  **2026-09-02 이전 릴리스(≤1.6.8)는 MIT 그대로 유지**(소급 취소 없음, LICENSE.md 명시).
- 랜딩 페이지·리포 가이드·데스크톱 매니페스트의 잔존 MIT 표기 정리 (#95).
